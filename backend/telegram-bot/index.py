...
def save_user_data(user_id: int, user_name: str, birth_date: str, db_url: str):
    if not db_url:
        return
    conn = psycopg2.connect(db_url)
    cur = conn.cursor()
...
def handle_message(message: Dict, bot_token: str, db_url: str):
    ...
    if text == '/start':
        ...
        if user_data and user_data['birth_date'] and not user_data['user_name']:
            save_user_data(user_id, text, user_data['birth_date'].isoformat(), db_url)
...
def handle_callback(callback: Dict, bot_token: str, db_url: str):
    ...
    if data == 'confirm':
        ...
        if user_data:
            ...
            keyboard = {
                'inline_keyboard': [
                    [{'text': '🔄 Обновить', 'callback_data': 'realtime'}],
                    [{'text': '❌ Скрыть', 'callback_data': 'back'}]
                ]
            }
            edit_message(chat_id, message_id,
                ...
                bot_token, keyboard)
        answer_callback(callback['id'], bot_token, '✅ Обновлено')
